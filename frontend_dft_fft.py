            ...
            st.code(
                """
                    ...
                    def DFT_FFT_magnitude_norm(self, X, fs):
                        ...
                        X_magnitude_norm = np.concatenate(
                            (X_magnitude[0] / 2, X_magnitude[1:] / (N / 2))
                        )
                        return {"DFT Magnitude": X_magnitude_norm, "Frequency": f}
                    """
            )
            st.markdown(
                """
    ...
    def one_c_frontend(self):
        ...
        with st.expander("1 c"):
            with st.form("one_c_form"):
                ...
                one_c_state["Submitted"] = st.form_submit_button("Run", on_click=self._run_one_c(one_c_state))
    
    
    def one_d_frontend(self):
        st.markdown("- Do same task with sequences of 1024 and 2048 sample length. Write discussions about some important points about sample length and period of signal in case of  time to frequency domain transformation in discrete time data using DFT/FFT.")
        with st.expander("1 d"):
            with st.form("one_d_form_1"):
                ...
                one_d_state_1["Submitted"] = st.form_submit_button("Run", on_click=self._run_one_c(one_d_state_1))
            ...
            with st.form("one_d_form_2"):
                ...
                one_d_state_2["Submitted"] = st.form_submit_button("Run", on_click=self._run_one_c(one_d_state_2))
    ...
    def two_a_frontend(self):
        ...
        with st.expander("2 a"):
            ...
            if two_a_state["Preset Waves"] == "1":
                preset=[int(2.381*fs),int(2.514*fs),int(2.572*fs),int(2.656*fs),int(2.894*fs),int(3.131*fs)]
            elif two_a_state["Preset Waves"] == "2":
                preset=[0,0,0,0,0,0]
            elif two_a_state["Preset Waves"] == "3":
                preset=[0,0,0,0,0,0]
            two_a_state["P Start"]=int(c1.number_input("P Start",min_value=0,max_value=21600,value=preset[0]))
            two_a_state["P End"] = int(c2.number_input("P End",min_value=0,max_value=21600,value=preset[1]))
            two_a_state["QRS Start"] = int(c1.number_input("QRS Start",min_value=0,max_value=21600,value=preset[2]))
            two_a_state["QRS End"] = int(c2.number_input("QRS End",min_value=0,max_value=21600,value=preset[3]))
            two_a_state["T Start"] = int(c1.number_input("T Start",min_value=0,max_value=21600,value=preset[4]))
            two_a_state["T End"] = int(c2.number_input("T End",min_value=0,max_value=21600,value=preset[5]))
